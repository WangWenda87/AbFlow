...
python -m models.pipeline.gen_complex_templates \
...
if [ "${MODEL}" = "MEAN" ]; then
    PYTHONPATH=$MODEL_CODE_DIR python models/pipeline/cdr_models/mean_gen.py \
        --ckpt ${CKPT} \
    ...
    PYTHONPATH=$CODE_DIR python models/pipeline/cdr_models/rosetta_gen.py \
        --dataset ${DATA_OUT_DIR}/summary.json \
        --cdr_type ${CDR} \
        --out_dir ${RESULT_DIR}
    ...
    PYTHONPATH=$MODEL_CODE_DIR python models/pipeline/cdr_models/diffab_gen.py \
        --dataset ${DATA_OUT_DIR}/summary.json \
        --out_dir ${RESULT_DIR} \
        --config ${CKPT}
    conda deactivate
...
if [ "${MODEL}" = "Rosetta" ]; then
    python -m models.pipeline.evaluate \
