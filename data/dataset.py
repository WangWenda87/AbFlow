...
def _generate_pep_data(cplx : AgAbComplex, workspace='workspace/', pdb_path=os.getcwd() + '/all_data/SAb-23-H2-Ab/pdb/') : 
    ...
    try :
        ...
        for res in pep_structure.residues : 
            ...
            # print(VOCAB.MAX_ATOM_NUMBER - len(full_atom_coords))
...
class E2EDataset(torch.utils.data.Dataset):
    def __init__(self, file_path, save_dir=None, pep_file=None, surf_file=None, cdr=None, paratope='H3', num_verts=75, full_antigen=False, num_entry_per_file=-1, random=False):
        '''
    ...
    def collate_fn(cls, batch):
        keys = ['X', 'S', 'smask', 'cmask', 'paratope_mask', 'residue_pos', 'template', 'xloss_mask', 'X_pep', 'S_pep', 'surface']
        types = [torch.float, torch.long, torch.bool, torch.bool, torch.bool, torch.long, torch.float, torch.bool, torch.float, torch.long, torch.float]
        # keys = ['X', 'S', 'smask', 'cmask', 'paratope_mask', 'residue_pos', 'template', 'xloss_mask', 'X_pep', 'S_pep']
        # types = [torch.float, torch.long, torch.bool, torch.bool, torch.bool, torch.long, torch.float, torch.bool, torch.float, torch.long]
        res = {}
