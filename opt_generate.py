...
def opt_generate(args):
    ...
    model.to(device)
    model.eval()
    predictor = torch.load(args.predictor_ckpt, map_location='cpu', weights_only=False)
    predictor.to(device)
    predictor.eval()
    ...
    for item_id, item in enumerate(items):
        ...
        std_score = round(np.std(scores, ddof=1), 3)
        ...
        message = f'{pdb_id}: mean ddg {mean_score}, std ddg {std_score}, best ddg {round(scores[best_score_idx], 3)}, diff cnt {different_cnt}, success rate {sucess_rate}, change: {cur_changes[best_score_idx]}, sample {gen_pdbs[best_score_idx]}\n'
