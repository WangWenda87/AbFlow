...
from evaluation.bak_dockq import dockq
...
def cal_metrics(inputs):
    ...
    # 5. DockQ
    try:
        dq = dockq(mod_cplx, ref_cplx, cdrh3_only=True) # consistent with HERN
...
def stat_metrics(pkl) : 
    ...
    for metric in metrics :
        for key in metric.keys() :
            metrics_dict[key].append(metric[key])
    
    dq_stat = get_proportions_above_cutoff(metrics_dict['DockQ'], [0.23, 0.49, 0.8])
    return metrics_dict

def get_proportions_above_cutoff(dq, cutoffs):
    result = []
    for cutoff in cutoffs:
        count_above = sum(1 for num in dq if num >= cutoff)
        proportion = count_above / len(dq)
...
def main(args):
    ...
    if args.metrics_path :
        with open(args.metrics_path, 'wb') as fout:
    ...
    for name in metrics[0]:
        ...
        if name == 'DockQ':
            dq_stat = get_proportions_above_cutoff(vals, [0.23, 0.49, 0.8])
            print(f'\tproportion of DockQ above 0.23: {dq_stat[0]},0.49: {dq_stat[1]}, 0.8: {dq_stat[2]}')
        # if name == 'RMSD(CA) CDRH3'
...
def parse():
    ...
    parser.add_argument('--metrics_path', type=str, help='pkl file that save metrics summary')
