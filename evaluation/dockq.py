...
def dockq(mod_cplx: AgAbComplex, ref_cplx: AgAbComplex, cdrh3_only=False):
    ...
    if cdrh3_only:
        ...
        mod_chains = "".join(mod_cplx.get_chain_names())
        ref_chains = "".join(ref_cplx.get_chain_names())
        mod_cplx.to_pdb(mod_pdb)
        ref_cplx.to_pdb(ref_pdb)
        mapp = mod_chains + ':' + ref_chains
        p = os.popen(f'DockQ --mapping {mapp}  {mod_pdb} {ref_pdb}')
        text = p.read()
        p.close()
        res = re.search(r'DockQ:\s+([0-1]\.[0-9]+)', text)
        score = float(res.group(1))
        clash = re.search(r'clashes:\s+([0-9]+)', text)
        clash_score = float(clash.group(1))
        # os.remove(mod_pdb)
        # os.remove(ref_pdb)
        return score, clash_score
    else:
        mod_ag_chains = mod_cplx.antigen.get_chain_names()[0]
        ref_ag_chains = ref_cplx.antigen.get_chain_names()[0]
        mod_ab_chains = mod_cplx.antibody.get_chain_names()
        ref_ab_chains = ref_cplx.antibody.get_chain_names()
        mod_cplx.to_pdb(mod_pdb)
        ref_cplx.to_pdb(ref_pdb)
        dq = []
        for c in range(len(mod_ab_chains)):
            mapp = mod_ag_chains + mod_ab_chains[c] + ':' + ref_ag_chains + ref_ab_chains[c]
            p = os.popen(f'DockQ --mapping {mapp} {mod_pdb} {ref_pdb}')
            text = p.read()
            p.close()
            res = re.search(r'DockQ:\s+([0-1]\.[0-9]+)', text)
