...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, pdb, heavy_chain, light_chain, antigen_chains, num_residue_changes, cdr='H3') -> None:
        ...
        paratope = [self.cdr] if type(self.cdr) == str else self.cdr
        for cdr in paratope:
    ...
    def collate_fn(cls, batch):
        keys = ['X', 'S', 'smask', 'cmask', 'paratope_mask', 'residue_pos']
        types = [torch.float, torch.long, torch.bool, torch.bool, torch.bool, torch.long]
        res = {}
